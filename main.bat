...
IF %CAMS_STATE%==_5 IF %FOXY% EQU 3 SET CAMS_STATES=%CAMS_STATES%_foxy_3
...
IF %CHOICE.INPUT%==TIMEOUT (
	...
	IF %VIEW%==CAMS (
		...
		REM IF %CAMS_STATE%==_5 (
			REM IF NOT %OLD_FOXY% EQU !FOXY! IF !FOXY! GTR 0 IF !FOXY! LEQ 3 ENDLOCAL&GOTO :RE
		REM )
		IF %CAMS_STATE%==_6 (
			IF NOT %OLD_BONNIE% EQU !BONNIE! IF !BONNIE! EQU 3 ENDLOCAL&GOTO :RE
